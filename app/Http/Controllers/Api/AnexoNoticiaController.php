...
public function index()
{
    ...
    $anexos->transform(function ($anexo) {
        if ($anexo->anexo) {
            $anexo->anexo = asset($anexo->anexo); // transforma em URL completa
        }
        return $anexo; // <<< CORRIGIDO
    });
    ...
    /**
     * Display a listing of the resource with noticias correspondentes.
     */
    public function indexWithNoticias()
    {
        $anexos = AnexoNoticia::with('noticia')->get();
        return response()->json($anexos);
    }
    /**
...
public function showByNoticia($noticia_id)
{
    $anexos = AnexoNoticia::where('noticia_id', $noticia_id)->get();
    $anexos->transform(function ($anexo) {
        if ($anexo->anexo) {
            $anexo->anexo = asset($anexo->anexo); // transforma em URL completa
        }
